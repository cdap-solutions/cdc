...
When the plugin receives a DDL record, it creates a table in the target HBase database. Below is a sample DDL Record that creates a table in namespace `GGTEST` with name `TESTANOTHER`.
```{
  "schema": {
    "type": "RECORD",
...
The content of the changes is listed in the `change` field. The `primary_keys` field specifies which fields in `change` will be used to name a row in the table. Below is a sample DML record that creates a row for `Scott` and inserts his information into the row.
```
{
